...
holdbrTap_eth0tap0()
{
    ...
    if [ "${value_temp}" == "" ];then
		...
		value_temp=$(brctl show | grep eth0)
		if [ "${value_temp}" == "" ]
		then
			ip link set eth0 master brTap
			ip link set eth0 up
		fi
    fi
}
...
rm smart_car.log
nice -n -20 taskset -c 2 ./smart_car 123 2 >> smart_car.log &
#  ------------------------------------------------------------------
  # @brief       loop
#  ------------------------------------------------------------------*/
while [ 1 ]
do
    ...
    task_flag=$(ps -elf | grep smart_car | wc -l)
    #echo $task_flag
	if [ "$task_flag" != "2" ]; then
		echo "smart_car is stuck"
		killall smart_car
        sleep 1
	nice -n -20 taskset -c 2 ./smart_car 123 2 >> smart_car.log &
	fi  
